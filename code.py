...
def winner():
    for x in range(3):
        if grid[x][0] == 1 and grid[x][1] == 1 and grid[x][
            2] == 1:  # Loop through the rows and report if the player1 have won
            print('Player1 won!')
            return True
    for x in range(3):
        if grid[x][0] == 2 and grid[x][1] == 2 and grid[x][
            2] == 2:  # Loop through the rows and report if the player2 have won
            print('Player2 won!')
            return True
    for x in range(3):
        if grid[0][x] == 1 and grid[1][x] == 1 and grid[2][
            x] == 1:  # Loop through the columns and report if the player1 have won
            print('Player1 won!')
            return True
    for x in range(3):
        if grid[0][x] == 2 and grid[1][x] == 2 and grid[2][
            x] == 2:  # Loop through the columns and report if the player2 have won
            print('Player2 won!')
            return True
    if grid[0][0] == 1 and grid[1][1] == 1 and grid[2][
        2] == 1:  # Loop through the diagonals and report if the player1 have won
        print('Player1 won!')
        return True
    if grid[0][0] == 2 and grid[1][1] == 2 and grid[2][
        2] == 2:  # Loop through the diagonals and report if the player2 have won
        print('Player2 won!')
        return True
    return False
    ...
    while max_moves > 0:  # moves allowed before the game goes to an end
        if turn == 1:  # Player1 turn
            try:
                ...
                if grid[player1_row][player1_col] == 1 or grid[player1_row][
                    player1_col] == 2:  # Check if is a valid move. If not, play again
                    turn = 1
